...
### Stripe webhook (card decline → Seq)

When a card is **declined** or a payment fails, Stripe can send a webhook to the app. The app logs a **decline/failure message** to Serilog (Console, File, **Seq**) so you can see it in Seq.

- **Endpoint:** `POST /webhooks/stripe`
- **Configure:** Set the webhook signing secret. Use the secret from **the same place that sends the webhook**:
  - **Stripe Dashboard:** [Developers → Webhooks](https://dashboard.stripe.com/test/webhooks) → your endpoint → "Reveal" signing secret. Use that `whsec_...` value.
  - **Stripe CLI** (e.g. `stripe listen --forward-to https://your-url/webhooks/stripe`): Use the `whsec_...` secret that the CLI prints when it starts (not the Dashboard secret).
  ```bash
  cd SportsStore
  dotnet user-secrets set "Stripe:WebhookSecret" "whsec_your_actual_secret" --project SportsStore.csproj
  ```

- **Local testing with Stripe CLI (on your PC):** Install [Stripe CLI](https://stripe.com/docs/stripe-cli), run `stripe login`, then `stripe listen --forward-to http://localhost:5000/webhooks/stripe`. Use the `whsec_...` secret the CLI prints (set via user-secrets). Restart the app and do a test payment; events will hit the app and show in Seq.
- **Alternative (ngrok):** Expose the app with ngrok, add that URL in Stripe Dashboard → Webhooks, use that endpoint's "Reveal" signing secret.
- **What appears in Seq:** For `payment_intent.payment_failed` or `charge.failed`, the app logs:  
  `Stripe payment declined or failed. EventId: {StripeEventId}, EventType: {StripeEventType}, Message: ...`  
  so you can filter in Seq by level or by `StripeEventType`.
...
## GitHub Actions CI (Part D)

A workflow runs on **push** and **pull_request** to **main** and restores, builds, and runs tests. The pipeline **fails if tests fail**.

- **Workflow file:** `.github/workflows/ci.yml`
- **Triggers:** `on.push.branches: [main]`, `on.pull_request.branches: [main]`
- **Steps:** checkout → setup .NET 9 → `dotnet restore SportsSln.sln` → `dotnet build` → `dotnet test` (with JUnit logger for test reporting)
- **Requirement:** Push the repo to GitHub and ensure the default branch is `main` (or change `branches` in the workflow to match your branch name).
